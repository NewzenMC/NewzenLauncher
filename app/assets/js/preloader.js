...
function onDistroLoad(data) {
    if (data != null) {
        ...
        if (
            ...
            data.getServer(ConfigManager.getSelectedServer()) == null
        ) {
...
DistroManager.pullRemote()
    ...
    .catch((err) => {
        ...
        logger.log(
            'Attempting to load an older version of the distribution index.'
        )
        // Try getting a local copy, better than nothing.
        DistroManager.pullLocal()
            .then((data) => {
                logger.log(
                    'Successfully loaded an older version of the distribution index.'
                )
            ...
            .catch((err) => {
                logger.log(
                    'Failed to load an older version of the distribution index.'
                )
